...
const formattedLink = ref(props.displayLink || props.to);
...
const colorStyles: Record<
    Colors,
    { iconBox: string; icon: string; hoverBorder: string; hoverBg: string }
> = {
    neutral: {
    ...
    primary: {
        ...
        hoverBg:
            'group-hover:bg-primary-50/50 dark:group-hover:bg-primary-900/10',
    },
    secondary: {
        iconBox: 'bg-secondary-50 dark:bg-secondary-950/30',
        icon: 'text-secondary-500 dark:text-secondary-400',
        hoverBorder:
            'group-hover:border-secondary-200 dark:group-hover:border-secondary-800',
        hoverBg:
            'group-hover:bg-secondary-50/50 dark:group-hover:bg-secondary-900/10',
    },
    success: {
        ...
        hoverBg:
            'group-hover:bg-emerald-50/50 dark:group-hover:bg-emerald-900/10',
    },
    info: {
        iconBox: 'bg-sky-50 dark:bg-sky-950/30',
        icon: 'text-sky-500 dark:text-sky-400',
        hoverBorder:
            'group-hover:border-sky-200 dark:group-hover:border-sky-800',
    ...
    <ClientOnly>
        ...
        >
            <NuxtLink
                :to="to"
                :target="finalTarget"
                class="group relative w-full flex items-center p-3 rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 transition-all duration-300 ease-out hover:shadow-lg hover:-translate-y-0.5 overflow-hidden"
                :class="[currentStyle.hoverBorder, currentStyle.hoverBg]"
            >
                <!-- 左侧：图标容器 -->
                <div
                    class="shrink-0 flex items-center justify-center w-10 h-10 rounded-lg mr-4 transition-transform duration-300 group-hover:scale-110"
