...
const saveProfile = async () => {
    ...
    if (formState.password) {
        ...
        try {
            const response = await usePanelApi('post', '/players/me/password', {
    ...
    const oldProfile = cache.playerProfile;
    if (
        formState.online_qq_suffix !== undefined &&
        formState.online_qq_suffix != oldProfile?.online_qq_suffix
    ) {
        updatedProfile.online_qq_suffix = formState.online_qq_suffix;
        if (oldProfile)
            oldProfile.online_qq_suffix = formState.online_qq_suffix;
    }
    if (
        formState.offline_qq_suffix !== undefined &&
        formState.offline_qq_suffix != oldProfile?.offline_qq_suffix
    ) {
        updatedProfile.offline_qq_suffix = formState.offline_qq_suffix;
        if (oldProfile)
            oldProfile.offline_qq_suffix = formState.offline_qq_suffix;
    }
    if (
        formState.avatar_source !== undefined &&
        formState.avatar_source != oldProfile?.avatar_source
    ) {
    ...
    if (Object.keys(updatedProfile).length !== 0) {
        try {
            const response = await usePanelApi('patch', '/players/me/profile', {
                body: updatedProfile,
            });
            if (response.code === 200) {
...
const loadProfile = async () => {
    cache.playerProfile = undefined;
    const response: ApiResponse<PlayerProfile> = await usePanelApi(
        'get',
        '/players/me/profile',
    );
    if (response.code === 200) {
        ...
        cache.playerProfile =
            response.data === null ? undefined : response.data;
    }
};

onMounted(() => {
    if (cache.playerProfile === undefined) {
        loadProfile();
    }
});
</script>
    ...
    <div class="max-w-2xl mx-auto space-y-6">
        <UCard>
            ...
            <div class="space-y-6">
                ...
                <USkeleton
                    v-if="cache.playerProfile === undefined"
                ...
                >
                    <div class="flex space-x-6">
                        ...
                        >
                            <USelect
                                v-model="formState.avatar_source"
                                :items="avatarSourceItems"
                            />
                        </UFormField>
                    </div>
                    <div
                        v-if="
                            cache.playerProfile.social_accounts.qq !==
                                undefined &&
                            formState.online_qq_suffix !== undefined &&
                            formState.offline_qq_suffix !== undefined
                        "
                    ...
                    >
                        ...
                        <UTooltip
                            :delay-duration="0"
                            :open="showOnlineSuffixPrev"
                        >
                            <UFormField
                                label="在线QQ后缀"
                                name="online_qq_suffix"
                            >
                                <UInput
                                    v-model="formState.online_qq_suffix"
                                    placeholder="| {s}在线"
                            ...
                            <template #content>
                                {{
                                    formState.online_qq_suffix === ''
                                        ? `${user?.name} | Minecraft在线`
                        ...
                        <UTooltip
                            :delay-duration="0"
                            :open="showOfflineSuffixPrev"
                        >
                            <UFormField
                                label="离线QQ后缀"
                                name="offline_qq_suffix"
                            >
                                <UInput
                                    v-model="formState.offline_qq_suffix"
                                    placeholder="| {s}离线"
                                    @pointerenter="showOfflineSuffixPrev = true"
                                    @pointerleave="
                                        showOfflineSuffixPrev = false
                                    "
                                />
                            </UFormField>
                            <template #content>
                                {{
                                    formState.offline_qq_suffix === ''
                                        ? `${user?.name} | Minecraft离线`
