...
const { usePanelApi, useExternalApi } = useApi();
...
const profileState: Partial<ProfileSchema> = reactive<Partial<ProfileSchema>>(
    lockServerID
        ? JSON.parse(JSON.stringify(props.initProfile))
        : { server_id: '请填写服务器ID' },
);
...
async function onSubmit(event: FormSubmitEvent<ProfileSchema>) {
    ...
    loading.value = true;
    try {
        await usePanelApi('put', `/servers/${event.data.server_id}`, {
            body: event.data,
        });
    } catch {
        return;
    ...
    >
        <template #body>
            ...
            >
                ...
                >
                    <UInput
                        v-model="profileState.server_id"
                        :disabled="lockServerID"
                    />
                </UFormField>
                <div class="grid grid-cols-2 gap-6">
                    ...
                    >
                        <UInput
                            v-model="profileState.zh_cn_name"
                            class="w-full"
                            placeholder="为空则显示服务器ID"
                        >
                            <template
                                v-if="profileState.zh_cn_name?.length"
                                #trailing
                            >
                                <UButton
                                    color="neutral"
                    ...
                    >
                        <UInput
                            v-model="profileState.en_ww_name"
                            class="w-full"
                            placeholder="为空则显示服务器ID"
                        >
                            <template
                                v-if="profileState.en_ww_name?.length"
                                #trailing
                            >
                                <UButton
                                    color="neutral"
                ...
                <div class="grid grid-cols-2 gap-6">
                    ...
                    >
                        <USelect
                            v-model="profileState.mc_version"
                    ...
                    >
                        <UInput
                            v-model="profileState.server_version"
                            class="w-full"
                            placeholder="如整合包版本号"
                        >
                            <template
                                v-if="profileState.server_version?.length"
                                #trailing
                            >
                                <UButton
                                    color="neutral"
                                    ...
                                    icon="i-lucide-circle-x"
                                    aria-label="清空"
                                    @click="profileState.server_version = ''"
                                />
                            </template>
                        </UInput>
                    </UFormField>
                ...
                >
                    <UTextarea
                        v-model="profileState.announcement"
                        class="w-full"
                        autoresize
                    />
                </UFormField>
        ...
        <template #footer>
            <div class="flex gap-2">
                ...
                <UButton
                    label="保存"
                    type="submit"
                    form="server-profile-editor-form"
                    :disabled="loading"
                />
            </div>
        </template>
