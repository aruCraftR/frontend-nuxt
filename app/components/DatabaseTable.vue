...
<template>
    <div class="flex flex-col gap-4">
        <div class="border rounded-md dark:border-gray-700">
            <UTable :loading="props.loading" :data="props.data" :columns="displayColumns" :pagination="tablePagination"
                v-bind="$attrs">
        ...
        <div class="flex justify-end pt-2 items-center gap-4">
            <UPagination v-model:page="currentPage" :items-per-page="perPage" :total="totalCount"
                :disabled="props.loading" class="flex items-center gap-1" />

            <!-- 新增：跳转输入区域 -->
            <div class="flex items-center gap-2">
                <span class="text-sm text-gray-500">前往</span>
                <UInput v-model="jumpPageInput" type="number" min="1" :max="maxPage" :disabled="props.loading"
                    placeholder="页码" class="w-16" size="sm" @keydown.enter="handleJump" @blur="handleJump" />
