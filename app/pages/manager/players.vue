...
const playerTabelColumns: TableColumn<PlayerTabelItem>[] = [
    ...
    },
    {
        ...
        header: 'UUID',
        cell: ({ row }) =>
            `${(row.getValue('uuid') as string).slice(0, 13)}...`,
    },
    {
        accessorKey: 'mc_id',
        header: 'MC ID',
    ...
    },
    {
        ...
        header: '最近上线时间',
        cell: ({ row }) => {
            const lastOnline = row.getValue('last_online_date') as
                | string
                | undefined;
            if (lastOnline) {
...
const fetchData = async (page: number) => {
    ...
    try {
        const response: ApiResponse<PlayerTabelApiData> = await usePanelApi(
            'get',
            '/manager/players',
            {
                query: {
...
async function changePermission(item: PlayerTabelItem) {
    ...
    try {
        await usePanelApi('patch', `/players/${item.uuid}/permission`, {
            body: {
    ...
    <div class="flex justify-center">
        ...
        >
            ...
            <template #permission-cell="{ row }">
                <UPopover arrow>
                    <UBadge
                        ...
                        variant="subtle"
                        :color="permissionColor[row.original.permission]"
                    >
                    ...
                    <template #content>
                        <UForm
                            class="p-5 flex"
                            @submit="changePermission(row.original)"
                        >
                            <UFormField
                                label="更改为"
                            ...
                            >
                                <USelect
                                    v-model="setPermissionValue"
                                    :items="setPermissionItems"
                                />
                            </UFormField>
                            <UButton
                                label="确认"
            ...
            <template #is_bound-cell="{ row }">
                <span
                    :class="`font-semibold capitalize ${row.original.is_bound ? 'text-success' : 'text-error'}`"
                    >{{ row.original.is_bound ? '已绑定' : '未绑定' }}</span
                >
            </template>
            <template #qq_id-cell="{ row }">
                <UUser
                    v-if="row.original.social_accounts.qq !== undefined"
                    :name="String(row.original.social_accounts.qq)"
                    :avatar="{
